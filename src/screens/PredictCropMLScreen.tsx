...
import { mlLechugasService } from "../services/prediccionService" 
import { useNavigation } from "@react-navigation/native"

const screenWidth = Dimensions.get("window").width

const CICLO_VIDA_MAXIMO_DIAS = 100 
...
interface ResultadoML {
  ...
  curvaCrecimientoAltura: number[] 
  curvaCrecimientoArea: number[]
}

interface PredictCropMLScreenProps {
  navigation?: any
}

export default function PredictCropMLScreen({ navigation }: PredictCropMLScreenProps = {}) {
  ...
  const cargarDatosContexto = async () => {
    try {
      const datos = await mlLechugasService.obtenerContextoActual()
  ...
  const realizarPrediccion = async () => {
    ...
    try {
      const resultadoML = await mlLechugasService.predecirConML({
        diasAPredecir: dias,
      ...
      >
          ...
          <View style={styles.contextCard}>
            ...
            <View style={styles.metricsGrid}>
              <View style={styles.metricItem}>
                <Text style={styles.metricVal}>{datosActuales.alturaActual.toFixed(1)} cm</Text>
                <Text style={styles.metricLab}>{t("altura") || "Altura"}</Text>
              </View>
              <View style={styles.metricItem}>
                <Text style={styles.metricVal}>{datosActuales.areaActual.toFixed(0)} cm²</Text>
                <Text style={styles.metricLab}>{t("areaFoliar") || "Área Foliar"}</Text>
              </View>
              <View style={styles.metricItem}>
            ...
            <View style={styles.ambientGrid}>
              ...
              <Text style={styles.ambientPill}>💧 {datosActuales.humedad}% {t("humedad")}</Text>
        ...
        <View style={styles.inputCard}>
          ...
          <TextInput
            ...
            placeholder={`Escribe aquí (Máx: ${limiteDiasPrediccion})`}
          ...
          >
            ...
            <Text style={styles.predictButtonText}>
              {cargandoPrediccion ? t("procesando") || "Procesando Modelo..." : "Ejecutar Modelo ML"}
            </Text>
          </TouchableOpacity>
          ...
          <>
            <View style={styles.resultCard}>
              ...
              <View style={styles.resultRow}>
                <Text style={styles.resultLabel}>{t("alturaEsperada") || "Altura Esperada:"}</Text>
                <Text style={styles.resultValue}>{resultado.alturaFinal.toFixed(2)} cm</Text>
              </View>
              <View style={styles.resultRow}>
                <Text style={styles.resultLabel}>{t("areaEsperada") || "Área Esperada:"}</Text>
                <Text style={styles.resultValue}>{resultado.areaFinal.toFixed(1)} cm²</Text>
              </View>
              <View style={styles.resultRow}>
            ...
            {resultado.curvaCrecimientoAltura?.length > 0 && (
              <View style={styles.chartContainer}>
                <Text style={styles.chartTitle}>{t("curvaProyectadaAltura") || "Curva Proyectada (Altura)"}</Text>
                <LineChart
                  data={{
                    labels: resultado.curvaCrecimientoAltura.length === 1 ? ["Hoy", "Futuro"] : resultado.curvaCrecimientoAltura.map((_, i) => i === 0 ? "Hoy" : `+${i}`),
                    datasets: [{ 
                      data: resultado.curvaCrecimientoAltura.length === 1 
                        ? [resultado.curvaCrecimientoAltura[0], resultado.curvaCrecimientoAltura[0]] 
                        : resultado.curvaCrecimientoAltura 
                    }],
                  }}
            ...
            {resultado.curvaCrecimientoArea?.length > 0 && (
              <View style={styles.chartContainer}>
                <Text style={styles.chartTitle}>{t("curvaProyectadaArea") || "Curva Proyectada (Área)"}</Text>
                <LineChart
                  data={{
                    labels: resultado.curvaCrecimientoArea.length === 1 ? ["Hoy", "Futuro"] : resultado.curvaCrecimientoArea.map((_, i) => i === 0 ? "Hoy" : `+${i}`),
                    datasets: [{ 
                      data: resultado.curvaCrecimientoArea.length === 1 
                        ? [resultado.curvaCrecimientoArea[0], resultado.curvaCrecimientoArea[0]] 
                        : resultado.curvaCrecimientoArea 
                    }],
                  }}
                  width={screenWidth - 80}
                  height={220}
